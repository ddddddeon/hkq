...
var Server = function(config) {
  ...
  this.startServer = function() {
    ...
    var server = net.createServer(function(sock) {
      sock.on('data', function(d) {
        ...
        d.toString().split('\n').forEach(function(data) { 
          ...
          if (data[0] === 'DCQ') {
            this.declareQueue(data, sock);
            console.log(this.queues);
          } else if (data[0] === 'ENQ') {
            this.enqueue(data, sock);
          } else if (data[0] === 'DEQ') {
            this.dequeue(data, sock);
          } else {
            this.respond(sock, 'ERR malformed request\n');
          }
        }.bind(this));
      }.bind(this));
    ...
    server.on('listening', function() {
      console.log('+ accepting connections on port %d...', this.config.listenPort);
    }.bind(this))

    setInterval(this.dumpToDisk.bind(this), this.config.dumpInterval * 10000);

...
var queues = {}; /* TODO move to object constructor */
var config = {};

var setupConfig = function(conf) {
  ...
  if (!conf ||
      ...
      conf.constructor.toString().indexOf('Object') < 0) {
    try {
  ...
  conf.dumpPath = path.resolve(process.env.SFQ_DUMP_PATH || conf.dumpPath);
  conf.dumpInterval = parseInt(process.env.SFQ_DUMP_INTERVAL || conf.dumpInterval);
  conf.listenPort = parseInt(process.env.SFQ_LISTEN_PORT || conf.listenPort);
  
  if (!fs.existsSync(conf.dumpPath)) {
    console.log('X could not find dump file:', conf.dumpPath);
    process.exit(1);
  }

  if (isNaN(conf.dumpInterval)) {
    console.log('X invalid dumpInterval (%s)', conf.dumpInterval);
    process.exit(1);
  }
  
  if (isNaN(conf.listenPort)) {
    console.log('X invalid listenPort (%s)', conf.listenPort);
    process.exit(1);
  }
...
var loadFromDisk = function(conf) {

...
var setupConfig = function() {
  ...
  config.dumpPath = process.env.SFQ_DUMP_PATH || config.dumpPath;
  config.dumpInterval = process.env.SFQ_DUMP_INTERVAL || config.dumpInterval;
  config.listenPort = process.env.SFQ_LISTEN_PORT || config.listenPort;

  if (!fs.existsSync(config.dumpPath)) {
    console.log('X could not find dump file ' + config.dumpPath);
    process.exit(1);
  }
...
var loadFromDisk = function() {
  ...
  console.log('+ loading queues from ' + config.dumpPath + '...');
  ...
  try {
    ...
    console.log('X (' + err.code + ') could not open dump file ' + config.dumpPath); 
    process.exit(1);
  }
};

var dumpToDisk = function() {
  console.log('+ dumping queue data to ' + config.dumpPath + '...');

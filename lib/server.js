...
var Server = function(config) {
  ...
  var stopServer = function() {
    clearInterval(this.dumpInBackground);
    if (this.writeLock !== true) {
      console.log('+ exiting...');
      process.exit(0);
    } else {
      console.log(' + SIGINT received while writing queue data to disk. dumping again and exiting...');
      this.writeLock = false;
      dumpToDisk(function(err, dumped) {
        this.writeLock = true;
        if (err) {
  ...
  var startServer = function(cb) {
    ...
    this.server = net.createServer(parseRequest);
    ...
    this.dumpInBackground = setInterval(dumpAtInterval.bind(this),
                                  this.config.dumpInterval * 100);

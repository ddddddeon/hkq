...
var Server = function(config) {
  ...
  var enqueue = function(data, sock) {
    ...
    if (typeof data[0] === 'undefined' ||
      ...
      data = data.join(' ').trim();
      q.enqueue(data);
      var message = 'EOK ' + q.messages.length + ' ' + data + '\n';
  ...
  var dequeue = function(data, sock) {
    ...
    if (typeof data[0] === 'undefined' ||
      ...
      if (q.messages.length < 1) {
        ...
        var message = 'DOK ' + q.messages.length + ' ' + item + '\n';
        respond(sock, message, function(err) {
          if (err) {
            console.log('X (%s) socket write error!');
            console.log('+ returning %s to front of queue %s', err.code, item, q.name);
            q.messages[q.messages.length] = item;
            console.log('+ "%s" is now at front of queue', q.messages[q.messages.length]);
          }
        });
      }
    }

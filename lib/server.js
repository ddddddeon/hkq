...
var Server = function(config) {
  ...
  var serverError = function(err) {
    if (err.code === 'EACCES') {
      ...
      dumpToDisk(function(err, dumped) {
        if (err) {
          console.log('X (%s) could not dump to disk... exiting', err.code);
        } else {
          console.log('+ dumped queues to disk... exiting');
        }
        process.exit(1);
      });
    }
  ...
  var startServer = function() {
    ...
    setInterval((function(ctx) {
      return function() {
        dumpToDisk(function(err, data) {
          if (err) {
            console.log('X (%s) could not dump to disk!', err.code);
          }
        });
      };
    })(this), this.config.dumpInterval * 60000);
  }.bind(this);
